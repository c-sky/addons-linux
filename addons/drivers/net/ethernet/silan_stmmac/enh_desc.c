...
#else
	for (i = 0; i < ring_size; i++) {
		GMAC_DESC0_SET(p, DESC0_ERX_OWN);
		GMAC_DESC1_WRITE(p, DESC1_ERX_BUFFER1_SIZE, DESC1_ERX_MASK_BUFFER1_SIZE, BUF_SIZE_8KiB - 1);
		/* To support jumbo frames */
		GMAC_DESC1_WRITE(p, DESC1_ERX_BUFFER2_SIZE, DESC1_ERX_MASK_BUFFER2_SIZE, BUF_SIZE_8KiB - 1);
		if (i == ring_size - 1)
			GMAC_DESC1_SET(p, DESC1_ERX_END_RING);
		if (disable_rx_ic)
			GMAC_DESC1_SET(p, DESC1_ERX_DISABLE_IC);
		p++;
	}
...
static void enh_desc_init_tx_desc(struct dma_desc *p, unsigned int ring_size)
{
	...
	for (i = 0; i < ring_size; i++) {
		...
		p->des01.etx.own = 0;
#else
		GMAC_DESC0_CLR(p, DESC0_ETX_OWN);
#endif
		if (i == ring_size - 1)
#ifdef GMAC_USE_BITFIELD
			p->des01.etx.end_ring = 1;
#else
			GMAC_DESC0_SET(p, DESC0_ETX_END_RING);
#endif
		p++;
	}
}
...
static void enh_desc_set_tx_owner(struct dma_desc *p)
{
	...
	p->des01.etx.own = 1;
#else
	GMAC_DESC0_SET(p, DESC0_ETX_OWN);
#endif
}
...
static void enh_desc_release_tx_desc(struct dma_desc *p)
{
	...
	*((volatile u32 *)p) = 0; 
	*((volatile u32 *) p + 1 ) = 0;
	GMAC_DESC0_WRITE(p, DESC0_ETX_END_RING, DESC0_ETX_MASK_END_RING, ter);
#endif
}
...
#else
	GMAC_DESC0_WRITE(p, DESC0_ETX_FIRST_SEGMENT, DESC0_ETX_MASK_FIRST_SEGMENT, is_fs);
	if (unlikely(len > BUF_SIZE_4KiB)) {
		GMAC_DESC1_WRITE(p, DESC1_ETX_BUFFER1_SIZE, DESC1_ETX_MASK_BUFFER1_SIZE, BUF_SIZE_4KiB);
		GMAC_DESC1_WRITE(p, DESC1_ETX_BUFFER2_SIZE, DESC1_ETX_MASK_BUFFER2_SIZE, len - BUF_SIZE_4KiB);

	} else {
		GMAC_DESC1_WRITE(p, DESC1_ETX_BUFFER1_SIZE, DESC1_ETX_MASK_BUFFER1_SIZE, len);
	}
	if (likely(csum_flag))
		GMAC_DESC0_WRITE(p, DESC0_ETX_CHECKSUM_INSERTION, DESC0_ETX_MASK_CHECKSUM_INSERTION, cic_full);
#endif
}
...
#else
	GMAC_DESC0_CLR(p, DESC0_ETX_INTERRUPT);
#endif
}
...
#else
	GMAC_DESC0_SET(p, DESC0_ETX_LAST_SEGMENT);
	GMAC_DESC0_SET(p, DESC0_ETX_INTERRUPT);
#endif
}
